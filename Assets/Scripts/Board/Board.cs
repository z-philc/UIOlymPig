...
public class Board : MonoBehaviour
{
    ...
    public void ShowEffectAtTileClicked(Tile tile)
    {
        if (clickEffect != null) Destroy(clickEffect);
        GamePiece clickedPiece = allGamePieces[tile.xIndex, tile.yIndex];
        clickEffect = Instantiate(goClickEffect, clickedPiece.transform.position, Quaternion.identity);
    }
        ...
        if (playerInputEnabled && !GameManager.Instance.IsGameOver)
        {
            ...
            if (targetPiece != null && clickedPiece != null)
            {
                ...
                if (targetEffect == null)
                    targetEffect = Instantiate(goClickEffect, targetPiece.transform.position, Quaternion.identity);
                ...
                if (tileBMatches.Count == 0 && tileAMatches.Count == 0 && colorMatches.Count == 0)
                {
                    ...
                    if (clickEffect != null) Destroy(clickEffect);
                    if (targetEffect != null) Destroy(targetEffect);
                }
                else
                {
